...
export default function VoiceControls({ voice }) {
  ...
  const micLabel = voice.requesting ? 'HAZIRLANIYOR' : voice.permissionState === 'denied' ? 'İZİN KAPALI' : !voice.active && permissionNeeded ? 'MİKROFON İZNİ VER' : speaking ? 'KONUŞUYOR' : voice.active ? 'MİKROFON AÇIK' : 'MİKROFON KAPALI';
  ...
  return <div className="relative flex items-center gap-1">
    <button onClick={voice.toggleMute} disabled={voice.remoteMuted || voice.requesting} className={`${control} ${voice.active ? 'text-green-400' : 'text-white/80'}`}>
      <span className={`flex h-7 w-7 items-center justify-center rounded-full border ${voice.active ? 'border-green-400' : 'border-white/20'} ${speaking ? 'speaking-glow' : ''}`}>{voice.active && !voice.remoteMuted ? <Mic className="w-4 h-4" /> : <MicOff className="w-4 h-4" />}</span>
