...
export default function RoomDirectMessages({ onClose, initialUserId }) {
  ...
  if (selected) {
    ...
    return <section className="h-full min-h-0 bg-card pt-[max(env(safe-area-inset-top),1rem)] pb-[env(safe-area-inset-bottom)]"><ChatPanel conversation={selected} userId={user.id} onBack={() => setSelected(null)} online={isOnline(friendId)} embedded /></section>;
  }
