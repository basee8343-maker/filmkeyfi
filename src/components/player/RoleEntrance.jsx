...
export default function RoleEntrance({ roomId, joinTrigger = 0, userId }) {
  ...
  const processMessage = useCallback((message) => {
    ...
    setQueue((items) => [...items.slice(-4), { key: message.id, frame: match[1], scale: Number(match[2]) || 100, avatar: message.user_avatar || '', name: message.user_name || 'Kullanıcı', hideName }]);
  }, [roomId]);
