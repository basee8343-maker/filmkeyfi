...
import ProfileCircleEntrance from '@/components/player/ProfileCircleEntrance';
import { CIRCLE_THEMES } from '@/lib/circleThemes';
...
export default function RoleEntrance({ roomId, joinTrigger = 0 }) {
  ...
  const processMessage = (msg) => {
    ...
    const circleTheme = CIRCLE_THEMES[roleKey];
    const isCircle = !!circleTheme;
    const useRoleName = ['founder', 'admin_helper', 'queen_admin'].includes(roleKey);
    const suffix = isCircle ? (isEntry ? 'cin' : 'cout') : (isEntry ? 'rin' : 'rout');
    setQueue((q) => [...q.slice(-4), {
      key: msg.id + suffix,
      type: isCircle ? 'circle' : 'role',
      roleKey,
      color: roleParsed.color || roleDef?.color || '#8b5cf6',
      isEntry,
      displayName,
      avatar: msg.user_avatar || '',
      roleLabel: isCircle ? roleLabel : (roleDef?.show_in_room ? roleLabel : ''),
      roleIcon: roleDef?.icon || '✨',
      hideUsername: roleDef?.hide_username_entry || false,
      useRoleName,
      circleThemeKey: roleKey,
    }]);
  };
  ...
  useEffect(() => {
    ...
    setCurrent(next);
    const duration = next.type === 'frame' ? (next.isEntry ? 3500 : 2100) : next.type === 'circle' ? (next.isEntry ? 4000 : 3000) : (next.isEntry ? 4500 : 4000);
