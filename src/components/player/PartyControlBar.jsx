import { MessagesSquare, MessageSquare, Settings } from 'lucide-react';
import VoiceControls from '@/components/player/VoiceControls';

export default function PartyControlBar({ voice, voiceEnabled, unread, directUnread, settingsOpen, chatOpen, directOpen, onSettings, onChat, onDirect, visible = true }) {
  ...
  return (
    <div className={`absolute inset-x-0 bottom-0 z-50 flex w-full items-center gap-3 border-t border-white/10 bg-black/95 px-4 py-2 backdrop-blur-xl sm:px-6 transition-transform duration-300 ${visible ? 'translate-y-0' : 'translate-y-[calc(100%+1rem)]'}`} style={{ paddingBottom: 'calc(0.5rem + env(safe-area-inset-bottom))' }}>
      ...
      <button onClick={onChat} className={`${control} ${chatOpen ? 'bg-white/15 text-white' : ''}`}><MessageSquare className="w-5 h-5" /><span className="text-[8px] font-bold">SOHBET</span>{unread > 0 && <span className="absolute right-0 top-0 min-w-4 rounded-full bg-primary px-1 text-[9px] font-bold text-primary-foreground">{unread > 99 ? '99+' : unread}</span>}</button>
