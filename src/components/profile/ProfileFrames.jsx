...
export default function ProfileFrames({ user, onUpdated }) {
  ...
  return <section className="rounded-2xl border border-border bg-card p-5">
    ...
    {!keys.length ? <p className="text-sm text-muted-foreground">Henüz açılmış çerçeven yok.</p> : <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
      {keys.map((key) => { const info = FRAME_DEFINITIONS[key]; return <button key={key} disabled={!!saving} onClick={() => choose(key)} className={`rounded-xl border p-2 text-center disabled:opacity-50 ${user.profile_frame === key ? 'border-primary bg-primary/10' : 'border-border bg-secondary/30'}`}><div className="mx-auto flex h-28 items-center justify-center"><ProfileFrame frame={key} avatar={user.avatar} name={user.username || user.full_name} size="md" frameScale={zoom} panX={user.profile_avatar_x} panY={user.profile_avatar_y} /></div><span className="text-xs font-semibold">{info.label}{user.profile_frame === key ? ' · Takılı' : ''}</span></button>; })}
    </div>}
    {user.profile_frame && <div className="mt-4 flex flex-wrap items-center gap-4 rounded-xl border border-border bg-secondary/30 p-3">
      <div className="flex h-20 w-20 shrink-0 items-center justify-center overflow-visible"><ProfileFrame frame={user.profile_frame} avatar={user.avatar} name={user.username || user.full_name} size="sm" frameScale={zoom} panX={user.profile_avatar_x} panY={user.profile_avatar_y} /></div>
