...
export default function RoleLabelEditor({ onUpdated }) {
  ...
  const overriddenCount = Object.values(labels).filter(Boolean).length;
    ...
    <div className="bg-card border border-border rounded-xl overflow-hidden">
      <button onClick={() => setOpen((v) => !v)} className="flex w-full items-center gap-3 p-4 text-left">
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground">{overriddenCount > 0 ? `${overriddenCount} özel isim` : 'Varsayılan isimler'}</p>
        </div>
        <ChevronDown className={`w-5 h-5 text-muted-foreground transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-2 border-t border-border pt-3">
          <p className="text-sm text-muted-foreground mb-2">Rollerin görünen adlarını değiştirebilirsiniz. Değişiklikler tüm platformda anlık uygulanır.</p>
          {roles.map(([key, info]) => {
              ...
              <div key={key} className="flex items-center gap-2">
                ...
                {isEditing ? (
                  <div className="flex items-center gap-1 shrink-0">
                    <button onClick={() => saveLabel(key)} disabled={savingKey === key} className="px-3 py-1.5 rounded-lg bg-primary text-primary-foreground text-xs font-semibold hover:bg-primary/90 disabled:opacity-50">{savingKey === key ? '...' : 'Kaydet'}</button>
                    <button onClick={() => { const ne = { ...editing }; delete ne[key]; setEditing(ne); }} className="px-2 py-1.5 rounded-lg bg-secondary text-xs font-semibold hover:bg-secondary/80">İptal</button>
                  </div>
                ) : (
                  <button onClick={() => setEditing({ ...editing, [key]: currentLabel })} className="px-3 py-1.5 rounded-lg bg-secondary text-xs font-semibold hover:bg-secondary/80 shrink-0">Değiştir</button>
                )}
                {isOverridden && !isEditing && (
                  <button onClick={() => { setEditing({ ...editing, [key]: '' }); setTimeout(() => saveLabel(key), 50); }} className="px-2 py-1.5 rounded-lg bg-destructive/15 text-destructive text-xs font-semibold hover:bg-destructive/25 shrink-0" title="Varsayılana sıfırla">Sıfırla</button>
                )}
              </div>
            );
          })}
