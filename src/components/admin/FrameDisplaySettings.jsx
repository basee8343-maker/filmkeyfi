...
export default function FrameDisplaySettings({ user, onUpdated }) {
  ...
  const adjust = (delta) => setScale((s) => Math.min(180, Math.max(80, s + delta)));
  ...
  return <div className="w-full rounded-xl border border-border bg-secondary/30 p-3">
    <div className="flex flex-wrap items-center gap-4"><div className="flex h-24 w-24 shrink-0 items-center justify-center overflow-visible">{user.profile_frame ? <ProfileFrame frame={user.profile_frame} avatar={user.avatar} name={user.username || user.full_name} size="md" frameScale={scale} /> : <span className="text-xs text-muted-foreground">Çerçeve yok</span>}</div><div className="min-w-[190px] flex-1"><div className="mb-1 flex justify-between text-xs font-semibold"><span>Profil yakınlaştırma</span><span>%{scale}</span></div><div className="flex items-center gap-2"><button onClick={() => adjust(-5)} className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-secondary border border-border"><Minus className="w-3.5 h-3.5" /></button><input type="range" min="80" max="180" step="1" value={scale} onChange={(e) => setScale(Number(e.target.value))} className="flex-1 accent-primary" /><button onClick={() => adjust(5)} className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-secondary border border-border"><Plus className="w-3.5 h-3.5" /></button></div><p className="mt-1 text-[10px] text-muted-foreground">Çerçeve sabit kalır, sadece profil fotoğrafı yakınlaşır.</p><button disabled={saving} onClick={() => save()} className="mt-2 rounded-lg bg-primary px-3 py-2 text-xs font-semibold text-primary-foreground disabled:opacity-50">Yakınlaştırmayı Kaydet</button></div></div>
