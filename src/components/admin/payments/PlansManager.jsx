...
export default function PlansManager() {
  ...
  useEffect(() => {
    load();
    const unsub = base44.entities.SubscriptionPlan.subscribe(() => load());
    return () => unsub();
  }, []);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => { setEditing(null); setShowForm(true); }} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold text-white shrink-0" style={{ background: 'linear-gradient(135deg, #7c3aed, #6b21a8)' }}>
      ...
      {plans.length === 0 ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {plans.map((p) => (
            <div key={p.id} className="bg-[#16161e] border border-white/5 rounded-xl p-4 flex flex-col">
              <div className="flex items-start justify-between mb-2 gap-2">
                ...
                <button onClick={() => toggleActive(p)} className="w-11 h-6 rounded-full relative overflow-hidden transition-colors shrink-0 mt-0.5" style={{ background: p.active ? '#22c55e' : '#ef4444' }}>
                  <span className="absolute top-0.5 left-0 w-5 h-5 rounded-full bg-white transition-transform" style={{ transform: `translateX(${p.active ? 22 : 2}px)` }} />
                </button>
              </div>
    ...
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-[#16161e] border border-white/10 rounded-xl p-5 w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={submit} className="space-y-3">
          <div><label className={labelClass}>Paket Adı</label><input value={name} onChange={(e) => setName(e.target.value)} required className={inputClass} /></div>
          <div><label className={labelClass}>Açıklama</label><textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={2} className={inputClass} /></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className={labelClass}>Fiyat (₺)</label><input type="number" value={price} onChange={(e) => setPrice(e.target.value)} required className={inputClass} /></div>
            <div><label className={labelClass}>Gün Sayısı</label><input type="number" value={durationDays} onChange={(e) => setDurationDays(e.target.value)} required className={inputClass} /></div>
          </div>
          <div><label className={labelClass}>Süre Etiketi (örn: 1 Ay, 30 Gün)</label><input value={durationLabel} onChange={(e) => setDurationLabel(e.target.value)} placeholder="30 Gün" className={inputClass} /></div>
          <div><label className={labelClass}>Özellikler (her satır bir özellik)</label><textarea value={features} onChange={(e) => setFeatures(e.target.value)} rows={4} placeholder="Film ve dizilere erişim&#10;Oda oluşturma" className={inputClass} /></div>
          ...
          <label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={active} onChange={(e) => setActive(e.target.checked)} className="w-4 h-4 accent-purple-500 shrink-0" /><span className="text-sm text-gray-300">Aktif</span></label>
