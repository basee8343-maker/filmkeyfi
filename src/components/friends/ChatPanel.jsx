...
export default function ChatPanel({ friendship, userId, invoke, onBack, online }) {
  ...
  const onPhoto = async (e) => {
    ...
    try {
      ...
      const dmFriendId = friendship.requester_id === userId ? friendship.recipient_id : friendship.requester_id;
      await base44.entities.Report.create({ reporter_id: userId, reporter_name: currentUser?.username || currentUser?.full_name || 'Kullanıcı', target_id: dmFriendId, target_name: name, context: 'dm', context_id: friendship.id, reason: 'Görsel onay', file_url, status: 'pending' });
