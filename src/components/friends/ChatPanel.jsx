...
import { ArrowLeft, Ban, Send, Settings, Trash2, Trash, Flag, X, Image as ImageIcon, Check, CheckCheck, Copy, UserMinus } from 'lucide-react';
...
export default function ChatPanel({ conversation, userId, onBack, online, embedded }) {
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(new CustomEvent('social-thread-open', { detail: { conversationId: conversation.id } }));
    markRead();
  ...
  const clearChat = async () => { setClearing(true); try { await deleteConversation(); setMenuOpen(false); } catch {} finally { setClearing(false); } };
  const unfriend = async () => {
    if (!friendship) { toast({ title: 'Arkadaşlık kaydı bulunamadı', variant: 'destructive' }); return; }
    setUnfriending(true);
    try {
      await base44.functions.invoke('friend-service', { action: 'unfriend', friendship_id: friendship.id });
      toast({ title: 'Arkadaş kaldırıldı' });
      onBack();
  ...
  return <section style={embedded ? undefined : { '--chat-height': `${viewport.height}px`, '--chat-top': `${viewport.top}px` }} className={embedded ? "relative inset-auto z-auto h-full min-h-0 flex flex-col bg-card" : "fixed inset-x-0 top-[var(--chat-top)] z-[60] h-[var(--chat-height)] min-h-0 flex flex-col overflow-hidden border-y border-border bg-card pt-[max(env(safe-area-inset-top),1.5rem)] sm:relative sm:inset-auto sm:z-auto sm:h-[72vh] sm:min-h-[72vh] sm:rounded-xl sm:border sm:pt-0"} onTouchStart={(e) => { touchStart.current = { x: e.touches[0].clientX, y: e.touches[0].clientY }; }} onTouchEnd={(e) => { const dx = e.changedTouches[0].clientX - touchStart.current.x; const dy = e.changedTouches[0].clientY - touchStart.current.y; if (dx > 80 && dx > Math.abs(dy) * 1.5) onBack(); }}>
    ...
    {menuOpen && <><div className="absolute inset-0 z-10" onClick={() => setMenuOpen(false)} /><div className="absolute z-20 right-3 top-14 w-52 rounded-xl border border-border bg-popover p-2 shadow-xl"><button onClick={() => { const v = !readReceiptsEnabled; localStorage.setItem('dm_read_receipts', String(v)); setReadReceiptsEnabled(v); }} className="w-full flex items-center justify-between rounded-lg px-3 py-2.5 text-sm font-semibold text-foreground hover:bg-secondary"><span className="flex items-center gap-2"><CheckCheck className={`w-4 h-4 ${readReceiptsEnabled ? 'text-blue-500' : 'text-muted-foreground'}`} />Okundu Tikleri</span><span className={`text-xs ${readReceiptsEnabled ? 'text-blue-500' : 'text-muted-foreground'}`}>{readReceiptsEnabled ? 'Açık' : 'Kapalı'}</span></button><div className="my-1 border-t border-border" /><button onClick={() => { setMenuOpen(false); setReportOpen(true); }} className="w-full flex items-center gap-2 rounded-lg px-3 py-2.5 text-sm font-semibold text-destructive hover:bg-destructive/10"><Flag className="w-4 h-4" />Şikayet Et</button><button onClick={clearChat} disabled={clearing} className="w-full flex items-center gap-2 rounded-lg px-3 py-2.5 text-sm font-semibold text-destructive hover:bg-destructive/10 disabled:opacity-50"><Trash className="w-4 h-4" />{clearing ? 'Siliniyor...' : 'Sohbeti Sil'}</button><button onClick={unfriend} disabled={unfriending || !friendship} className="w-full flex items-center gap-2 rounded-lg px-3 py-2.5 text-sm font-semibold text-destructive hover:bg-destructive/10 disabled:opacity-50"><UserMinus className="w-4 h-4" />{unfriending ? 'Kaldırılıyor...' : 'Arkadaşı Sil'}</button><button onClick={block} disabled={blocking || !friendship} className="w-full flex items-center gap-2 rounded-lg px-3 py-2.5 text-sm font-semibold text-destructive hover:bg-destructive/10 disabled:opacity-50"><Ban className="w-4 h-4" />{blocking ? 'Engelleniyor...' : 'Kullanıcıyı Engelle'}</button></div></>}
    <div ref={messagesRef} className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 space-y-4" style={{ touchAction: 'pan-y', WebkitOverflowScrolling: 'touch' }}><div className="rounded-xl bg-secondary/70 px-4 py-3 text-center text-xs text-muted-foreground">{name} ile sohbet</div>{loading && <p className="text-center text-sm text-muted-foreground py-12">Mesajlar yükleniyor...</p>}{!loading && !items.length && <p className="text-center text-sm text-muted-foreground py-12">İlk mesajı siz gönderin.</p>}{items.map((message) => { const sent = message.sender_id === userId; const senderRoleKey = sent ? (currentUser?.display_role || '') : (friendProfile?.display_role || ''); return <div key={message.id} className={`group flex items-end gap-2 ${sent ? 'justify-end' : 'justify-start'}`}>{!sent && (avatar ? <Image src={avatar} className="w-8 h-8 rounded-full shrink-0" fittingType="fill" /> : <div className="w-8 h-8 rounded-full bg-secondary flex items-center justify-center text-xs font-bold shrink-0">{name?.[0]}</div>)}<RoleMessageEffect roleKey={senderRoleKey} className="max-w-[78%]"><div className={`rounded-2xl px-4 py-2.5 text-sm ${sent ? 'rounded-br-md bg-primary text-primary-foreground' : 'rounded-bl-md bg-secondary text-foreground'}`}>{message.file_url && <Image src={message.file_url} alt="foto" className="rounded-lg max-w-[200px] max-h-56 object-cover mb-1 cursor-pointer" fittingType="fit" onClick={() => setLightbox(message.file_url)} />}{message.content && <p className="whitespace-pre-wrap break-words">{message.content}</p>}<p className="text-[10px] opacity-60 mt-1 text-right flex items-center justify-end gap-1">{new Date(message.created_date).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })}{sent && readReceiptsEnabled && ((message.read_by || []).includes(message.receiver_id) ? <CheckCheck className="w-3.5 h-3.5 text-blue-500" /> : <Check className="w-3.5 h-3.5" />)}</p></div></RoleMessageEffect>{(sent || isAdmin) && <button onClick={() => deleteMessage(message.id)} className="p-1 text-muted-foreground/50 hover:text-destructive shrink-0" title="Sil"><Trash2 className="w-3.5 h-3.5" /></button>}</div>; })}</div>
