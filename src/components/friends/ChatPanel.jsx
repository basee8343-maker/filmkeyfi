...
export default function ChatPanel({ conversation, userId, onBack, online, embedded }) {
  ...
  // Sohbete özel çevrim içi tercihini iki tarafta da anlık senkronla.
  useEffect(() => {
    if (!conversation?.id) return;
    setOfflineFor(conversation.offline_for || []);
    const unsub = base44.entities.Conversation.subscribe((event) => {
      if ((event.data?.id || event.id) !== conversation.id || !Array.isArray(event.data?.offline_for)) return;
      setOfflineFor(event.data.offline_for);
    });
    return unsub;
  }, [conversation?.id, conversation?.offline_for, userId]);
  ...
  const toggleOnline = async () => {
    ...
    setOfflineFor(nextEnabled ? previous.filter((id) => id !== userId) : [...new Set([...previous, userId])]);
    try {
