...
export default function ChatPanel({ conversation, userId, onBack, online, embedded, getLastSeen }) {
  ...
  useEffect(() => {
    ...
    if (!friendId) return;
    let active = true;
    const loadProfile = () => base44.functions.invoke('user-profile', { user_id: friendId })
      .then((res) => { if (active) setFriendProfile(res.data); })
      .catch((error) => { if (active && error?.response?.status === 404) setFriendProfile({ account_status: 'deleted' }); });
    ...
    const unsubUser = base44.entities.User.subscribe((event) => {
      if ((event.data?.id || event.id) === friendId) loadProfile();
    });
    return () => { active = false; unsub(); unsubUser(); };
  }, [conversation?.id, userId]);
  ...
  const friendLastSeen = getLastSeen ? getLastSeen(friendId) : null;
  ...
  const accountState = getAccountState(friendProfile, realtimeConversation?.last_message_text);
  const canMessage = !accountState && (isAdmin || friendProfile?.role === 'admin' || friendship?.status === 'accepted');
  ...
  return <section style={sectionStyle} className={embedded ? "relative inset-auto z-auto h-full min-h-0 flex flex-col bg-card" : "fixed top-0 left-0 right-0 bottom-0 z-[60] min-h-0 flex flex-col overflow-hidden border-y border-border bg-card sm:relative sm:inset-auto sm:z-auto sm:h-[72vh] sm:min-h-[72vh] sm:rounded-xl sm:border sm:pt-0"} onTouchStart={(e) => { touchStart.current = { x: e.touches[0].clientX, y: e.touches[0].clientY }; }} onTouchEnd={(e) => { const dx = e.changedTouches[0].clientX - touchStart.current.x; const dy = e.changedTouches[0].clientY - touchStart.current.y; if (dx > 80 && dx > Math.abs(dy) * 1.5) onBack(); }}>
    <header className="relative shrink-0 px-3 pt-[max(0.75rem,env(safe-area-inset-top))] pb-3 border-b border-border bg-card/95 backdrop-blur"><div className="flex items-center justify-between"><button onClick={onBack} className="p-2 rounded-full hover:bg-secondary" aria-label="Geri dön"><ArrowLeft className="w-6 h-6" /></button>{!accountState && <button onClick={() => setSettingsOpen(true)} className="p-2 rounded-full hover:bg-secondary" aria-label="Sohbet ayarları"><Settings className="w-5 h-5" /></button>}</div><div className={`flex flex-col items-center gap-1 ${accountState ? '' : 'cursor-pointer active:opacity-70'}`} onClick={() => { if (!accountState) setProfilePopup(true); }}>{accountState ? <AccountStatusAvatar status={friendProfile.account_status} className="h-12 w-12" /> : friendProfile?.profile_frame ? <ProfileFrame frame={friendProfile.profile_frame} avatar={friendProfile.avatar || avatar} name={name} size="sm" frameScale={friendProfile.profile_frame_scale} /> : <XpAvatar avatar={friendProfile?.avatar || avatar} name={name} userId={friendId} size="md" />}<h2 className="font-bold text-sm truncate max-w-[70%]">{accountState?.name || name}</h2><p className={`text-[11px] ${accountState ? 'text-destructive' : friendTyping || friendVisibleOnline ? 'text-green-500' : 'text-muted-foreground'}`}>{accountState ? accountState.message : friendTyping ? `${name} yazıyor...` : online ? '• Çevrim içi' : friendLastSeen ? `Son görülme: ${new Date(friendLastSeen).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })}` : 'Çevrim dışı'}</p></div>
    </header>
...
{friendship?.status === 'rejected' && <div className="rounded-xl bg-destructive/10 px-4 py-3 text-center text-xs text-destructive">Arkadaşlık isteğiniz reddedildi</div>}{friendship?.status === 'removed' && <div className="rounded-xl bg-secondary/70 px-4 py-3 text-center text-xs text-muted-foreground">Arkadaşlık kaldırıldı — tekrar eklemek için ayarlardan istek gönderin</div>}</div>
    <div className="shrink-0 p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] border-t border-border flex items-center gap-2 bg-card"><label className="p-2 rounded-lg hover:bg-secondary cursor-pointer shrink-0"><ImageIcon className="w-5 h-5" /><input type="file" accept="image/jpeg,image/png,image/webp" className="hidden" tabIndex={-1} aria-hidden="true" onChange={onPhoto} disabled={uploading} /></label><input value={text} onChange={(event) => changeText(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); handleSubmit(event); } }} disabled={!canMessage} maxLength={2000} enterKeyHint="send" inputMode="text" autoComplete="off" autoCorrect="off" autoCapitalize="sentences" spellCheck={false} placeholder={accountState ? accountState.message : canMessage ? 'Mesaj yazın...' : 'Yalnızca arkadaşlar mesajlaşabilir'} className="flex-1 min-w-0 bg-secondary rounded-full px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-ring disabled:opacity-60" /><button type="button" onClick={handleSubmit} disabled={!canMessage || sending || uploading || !text.trim()} className="w-11 h-11 shrink-0 flex items-center justify-center bg-primary text-primary-foreground rounded-full disabled:opacity-50" aria-label="Mesaj gönder"><Send className="w-5 h-5" /></button>{uploading && <span className="text-xs text-muted-foreground animate-pulse shrink-0">...</span>}</div>
    {profilePopup && !accountState && <UserProfile userId={friendId} embedded onBack={() => setProfilePopup(false)} onMessage={() => setProfilePopup(false)} />}
