...
import { ArrowLeft, Ban, Send, Settings, Trash2, Trash } from 'lucide-react';
import { Image } from '@/components/ui/image';
import { useCurrentUser } from '@/lib/useCurrentUser';
import useDirectMessages from '@/hooks/useDirectMessages';

export default function ChatPanel({ friendship, userId, invoke, onBack, online }) {
  ...
  return <section style={{ '--chat-height': `${viewport.height}px`, '--chat-top': `${viewport.top}px` }} className="fixed inset-x-0 top-[var(--chat-top)] z-[60] h-[var(--chat-height)] min-h-0 flex flex-col overflow-hidden border-y border-border bg-card pt-[max(env(safe-area-inset-top),1.5rem)] sm:relative sm:inset-auto sm:z-auto sm:h-[72vh] sm:min-h-[72vh] sm:rounded-xl sm:border sm:pt-0">
    ...
    {menuOpen && <div className="absolute z-20 right-3 top-14 w-52 rounded-xl border border-border bg-popover p-2 shadow-xl"><button onClick={clearChat} disabled={clearing || !allMessages.length} className="w-full flex items-center gap-2 rounded-lg px-3 py-2.5 text-sm font-semibold text-destructive hover:bg-destructive/10 disabled:opacity-50"><Trash className="w-4 h-4" />{clearing ? 'Temizleniyor...' : 'Tümünü Sil'}</button><button onClick={block} disabled={blocking} className="w-full flex items-center gap-2 rounded-lg px-3 py-2.5 text-sm font-semibold text-destructive hover:bg-destructive/10 disabled:opacity-50"><Ban className="w-4 h-4" />{blocking ? 'Engelleniyor...' : 'Kullanıcıyı Engelle'}</button></div>}
    <div ref={messagesRef} className="min-h-0 flex-1 overflow-y-auto p-4 space-y-4"><div className="rounded-xl bg-secondary/70 px-4 py-3 text-center text-xs text-muted-foreground">Artık {name} ile arkadaşsınız. Birbirinize özel mesaj gönderebilirsiniz.</div>{loading && <p className="text-center text-sm text-muted-foreground py-12">Mesajlar yükleniyor...</p>}{!loading && !items.length && <p className="text-center text-sm text-muted-foreground py-12">İlk mesajı siz gönderin.</p>}{items.map((message) => { const sent = message.sender_id === userId; return <div key={message.id} className={`group flex items-end gap-2 ${sent ? 'justify-end' : 'justify-start'}`}>{!sent && (avatar ? <Image src={avatar} className="w-8 h-8 rounded-full shrink-0" fittingType="fill" /> : <div className="w-8 h-8 rounded-full bg-secondary flex items-center justify-center text-xs font-bold shrink-0">{name?.[0]}</div>)}<div className={`max-w-[78%] rounded-2xl px-4 py-2.5 text-sm ${sent ? 'rounded-br-md bg-primary text-primary-foreground' : 'rounded-bl-md bg-secondary text-foreground'}`}><p className="whitespace-pre-wrap break-words">{message.text}</p><p className="text-[10px] opacity-60 mt-1 text-right">{new Date(message.created_date).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })}</p></div>{(sent || isAdmin) && <button onClick={() => del(message.id)} className="p-1 text-muted-foreground/50 hover:text-destructive shrink-0" title="Sil"><Trash2 className="w-3.5 h-3.5" /></button>}</div>; })}</div>
    <form onSubmit={handleSubmit} className="shrink-0 p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] border-t border-border flex items-center gap-2 bg-card"><input value={text} onChange={(event) => changeText(event.target.value)} maxLength={2000} placeholder="Mesaj yazın..." className="flex-1 min-w-0 bg-secondary rounded-full px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-ring" /><button type="submit" disabled={sending || !text.trim()} className="w-11 h-11 shrink-0 flex items-center justify-center bg-primary text-primary-foreground rounded-full disabled:opacity-50" aria-label="Mesaj gönder"><Send className="w-5 h-5" /></button></form>
  </section>;
}