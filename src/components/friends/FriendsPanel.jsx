      ...
      return <div key={r.id} className="border-b last:border-0 border-border p-3">
        <div className="flex items-center gap-3">
          <button onClick={() => onChat(r)} className="flex items-center gap-3 flex-1 min-w-0 text-left">
            ...
            <div className="flex-1 min-w-0"><p className="font-semibold truncate">{name}</p><p className="text-xs text-muted-foreground truncate">{friendRoom ? <span className="text-red-500 font-semibold">🔴 Film odasında</span> : online ? <span className="text-green-500">Çevrim içi</span> : 'Çevrim dışı'}{member ? ` · ${member}` : ''}</p></div>
          </button>
