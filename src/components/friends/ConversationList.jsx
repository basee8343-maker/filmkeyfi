...
export default function ConversationList({ relations, messages, userId, onOpen, onHide, isOnline }) {
  ...
  return <div>{chats.map(({ relation, latest }) => {
    ...
    return <div key={relation.id} className="relative overflow-hidden border-b border-border"><button onClick={() => onHide(relation)} className="absolute inset-y-0 right-0 w-20 bg-destructive text-destructive-foreground flex flex-col items-center justify-center text-xs font-semibold"><Trash2 className="w-5 h-5 mb-1" /> Sil</button><button onTouchStart={(e) => { startX.current = e.touches[0].clientX; }} onTouchEnd={(e) => setSwiped(startX.current - e.changedTouches[0].clientX > 55 ? relation.id : null)} onClick={() => swiped === relation.id ? setSwiped(null) : onOpen(relation)} className={`relative z-10 w-full flex items-center gap-3 bg-background px-4 py-4 text-left transition-transform ${swiped === relation.id ? '-translate-x-20' : 'translate-x-0'}`}>
      <div className="relative w-14 h-14 rounded-full bg-secondary shrink-0"><div className="w-full h-full rounded-full overflow-hidden">{avatar ? <Image src={avatar} alt={name} className="w-full h-full" /> : <span className="w-full h-full flex items-center justify-center text-xl font-bold">{name?.[0]}</span>}</div><span className={`absolute right-0 bottom-0 w-3.5 h-3.5 rounded-full border-2 border-background ${isOnline(relation.requester_id === userId ? relation.recipient_id : relation.requester_id) ? 'bg-green-500' : 'bg-muted-foreground'}`} /></div><div className="min-w-0 flex-1"><p className="font-semibold truncate">{name}</p><p className="text-xs text-muted-foreground">{isOnline(relation.requester_id === userId ? relation.recipient_id : relation.requester_id) ? 'Çevrim içi' : 'Çevrim dışı'}</p><p className="text-sm text-muted-foreground truncate">{latest?.text || 'Artık arkadaşsınız. Sohbeti başlatın.'}</p></div><div className="text-right shrink-0"><p className="text-xs text-muted-foreground">{latest ? new Date(latest.created_date).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' }) : ''}</p><ChevronRight className="w-4 h-4 ml-auto mt-2 text-muted-foreground" /></div>
    </button></div>;
  })}</div>;
}