...
export default function ConversationList({ conversations, userId, onOpen, onHide, isOnline, getLastSeen }) {
  ...
  return <div>{chats.map(({ conversation, name, avatar, otherId, unread }) => {
    ...
    const realOnline = isOnline ? isOnline(otherId) : false;
    const online = !friendOffline && realOnline;
    ...
    const displayName = accountState?.name || name;
    ...
    return <div key={conversation.id} className="relative overflow-hidden border-b border-border">
      ...
      >
        <div className="relative flex h-16 w-16 shrink-0 items-center justify-center">{accountState ? <AccountStatusAvatar status={profile.account_status} className="h-12 w-12" /> : profile?.profile_frame ? <ProfileFrame frame={profile.profile_frame} avatar={profile.avatar || avatar} name={name} size="sm" frameScale={profile.profile_frame_scale} /> : <XpAvatar avatar={profile?.avatar || avatar} name={name} userId={otherId} size="md" />}{unread > 0 && <span className="absolute -top-0.5 -right-0.5 min-w-5 h-5 px-1 rounded-full bg-primary text-primary-foreground text-[10px] font-bold flex items-center justify-center border-2 border-background">{unread}</span>}{!accountState && <span className={`absolute right-0 bottom-0 w-3.5 h-3.5 rounded-full border-2 border-background ${online ? 'bg-green-500' : 'bg-muted-foreground'}`} />}</div><div className="min-w-0 flex-1"><p className={`truncate ${unread > 0 ? 'font-bold text-foreground' : 'font-semibold text-muted-foreground'}`}>{displayName}</p><p className={`truncate text-xs ${accountState ? 'text-destructive' : typing ? 'font-semibold text-green-400' : 'text-muted-foreground'}`}>{accountState ? accountState.message : typing ? `${name} yazıyor...` : realOnline ? 'Çevrim içi' : lastSeen ? `Son görülme: ${new Date(lastSeen).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })}` : 'Çevrim dışı'}</p><p className={`text-sm truncate ${unread > 0 ? 'text-foreground font-medium' : 'text-muted-foreground'}`}>{unread > 0 ? <span className="text-primary">● </span> : ''}{accountState ? accountState.message : conversation.last_message_text || 'Sohbeti başlatın.'}</p></div><div className="text-right shrink-0 flex flex-col items-end gap-1"><p className="text-xs text-muted-foreground">{conversation.last_message_at ? new Date(conversation.last_message_at).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' }) : ''}</p>{!accountState && <FriendshipLevelBadge level={getProgression(otherId)?.level || 1} variant="list" />}<ChevronRight className="w-4 h-4 ml-auto text-muted-foreground" /></div>
      </div>
    </div>;
  })}</div>;
}