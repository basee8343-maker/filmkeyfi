...
export default function FriendshipLevelBadge({ level = 1, variant = 'compact', maxLevel = 1000 }) {
  ...
  if (variant === 'room') return <span className={`inline-flex max-w-full shrink-0 items-center justify-center gap-1 rounded-lg border px-2 py-1 text-[10px] font-black leading-4 ${theme.shell}`}>{theme.level >= 1000 && <Crown className="h-3 w-3 shrink-0" />}LVL {theme.level}</span>;
