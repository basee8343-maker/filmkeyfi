...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 pt-[max(env(safe-area-inset-top),1.5rem)] lg:pt-0"
      style={{ background: 'linear-gradient(135deg, #14141a 0%, #1a1525 50%, #14141a 100%)', borderBottom: '1px solid rgba(168,85,247,0.12)' }}>
            ...
            {links.map((l) => (
              <Link key={l.path} to={l.path}
                className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                  location.pathname === l.path ? 'text-white bg-purple-500/15' : 'text-white/60 hover:text-white hover:bg-white/5'}`}>
                {l.label}
              </Link>
            ))}
          </nav>
