...
// Seviye çerçevesi eşiklerine göre rozet metni — çerçevenin gerçek ismini gösterir.
const getLevelFrameLabel = (value) => {
  if (value >= 1000) return 'LVL MAX';
  if (value >= 750) return 'LVL 750';
  if (value >= 500) return 'LVL 500';
  if (value >= 250) return 'LVL 250';
  if (value >= 150) return 'LVL 150';
  if (value >= 75) return 'LVL 75';
  return `LVL ${value}`;
};
...
export default function RoomLevelBadge({ level, profile = false, textOnly = false }) {
  ...
  return <span data-level-tier={getRoomLevelTier(value)} data-level-motion={value % 8} className={`room-level-badge relative isolate inline-flex max-w-full shrink-0 items-center justify-center gap-1.5 overflow-hidden rounded-lg border font-bold leading-none ${profile ? 'room-level-profile px-4 py-2.5 text-base' : 'px-2 py-1.5 text-[10px]'}`} aria-label={`Seviye ${value}${value === 1000 ? ', maksimum seviye' : ''}`}>
    ...
    <span className="relative">{getLevelFrameLabel(value)}</span>
  </span>;
}