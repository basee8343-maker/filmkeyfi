...
export default function Friends() {
  ...
  return <div className="max-w-3xl mx-auto min-h-[calc(100vh-8rem)] bg-background"><header className="flex items-center px-4 py-5 border-b border-border"><h1 className="text-2xl font-extrabold">{view === 'friends' ? 'Arkadaşlar' : 'Sohbet'}</h1><button onClick={() => setView(view === 'friends' ? 'chats' : 'friends')} className="relative ml-auto w-11 h-11 rounded-full bg-yellow-400 text-black flex items-center justify-center" aria-label="Arkadaşlar">{view === 'friends' ? <X className="w-6 h-6" /> : <UsersRound className="w-6 h-6" />}{view !== 'friends' && unread > 0 && <span className="absolute -top-1 -right-1 min-w-5 h-5 px-1 rounded-full bg-destructive text-destructive-foreground text-[10px] font-bold flex items-center justify-center">{unread > 99 ? '99+' : unread}</span>}</button></header>
