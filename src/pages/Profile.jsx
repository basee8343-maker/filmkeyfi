...
export default function Profile() {
  const { user, reload } = useCurrentUser(); const { toast } = useToast();
  const [tab, setTab] = useState('info'); const [pkg, setPkg] = useState(null); const [historyMovies, setHistoryMovies] = useState([]); const [list, setList] = useState([]); const [favs, setFavs] = useState([]); const [editing, setEditing] = useState(false); const [form, setForm] = useState({ username: '', full_name: '', phone: '', avatar: '' });
  useEffect(() => {
    if (!user) return;
